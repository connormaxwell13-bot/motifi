...
function parseNotes(notes) {
  ...
  return notes.split(/\.\s*|;\s*/).map(s => s.trim()).filter(Boolean).filter(s => !/^none\s+significant/i.test(s))
}
...
export default function CarPage(props) {
  ...
  const fromResults = props.fromResults
  const onBack = props.onBack
  const onHome = props.onHome
  const onCompare = props.onCompare
  const onSelectCar = props.onSelectCar
    ...
    <div className="motifi-screen car">
      ...
      <div className="car-layout">
        <main className="car-main">
          ...
          <div ref={stockRef}><RetailerModule car={car} answers={answers} /></div>
          <div ref={insuranceRef}><InsuranceModule car={car} answers={answers} /></div>
          {isFinance && <div ref={financeRef}><FinanceModule car={car} answers={answers} cy={cy} /></div>}
          <KnownIssuesModule car={car} />
        </main>
        <aside className="car-rail">
          <PriceRail car={car} answers={answers} repPrice={repPrice} cy={cy} trueCost={trueCost} resale={resale} matchScore={matchScore} isFinance={isFinance} onSearchStock={() => scrollTo(stockRef)} onGetFinance={isFinance ? () => scrollTo(financeRef) : null} onGetInsurance={() => scrollTo(insuranceRef)} />
          <VerdictRail car={car} matchScore={matchScore} />
          {fromResults && results?.length > 1 && <OtherMatchesRail cars={results.filter(c => `${c.make}|${c.model}|${c.generationName}` !== `${car.make}|${car.model}|${car.generationName}`).slice(0, 4)} onSelectCar={onSelectCar} />}
        </aside>
      </div>
    </div>
  )
  ...
  return (
    <section className="car-hero">
      <div className="car-hero-l">
        <div className="car-hero-meta">◆ {(car.make || '').toUpperCase()} · {car.generationYears || ''} · {(car.bodyType || '').toUpperCase()}</div>
        <h1 className="car-hero-h1">{car.make} {car.model}</h1>
        <div className="car-hero-trim">{[car.generationName, car.fuelType, car.transmission].filter(Boolean).join(' · ')}</div>
        <div className="car-hero-chips">
          {chips.map(function(c, i) { return <span key={i} className="car-cap-chip">{c}</span> })}
        </div>
      ...
      <div className="car-hero-r">
        <div className="car-hero-photo">
          <img src={imaginUrl(car)} alt={`${car.make} ${car.model}`} onError={(e) => { e.currentTarget.style.opacity = '0.15' }} />
        </div>
      </div>
    </section>
  )
...
function CostBreakdown(props) {
  ...
  const answers = props.answers
  const cy = props.cy
  const isFinance = ['Hire Purchase', 'Hire Purchase (HP)', 'Personal Contract Purchase (PCP)'].includes(answers?.paymentMethod || answers?.purchaseMethod)
  ...
  const onRoadTotal = isFinance ? cy.deposit + cy.financeMonthly + cy.insuranceMonthly + cy.roadTax : (cy.effectivePrice ?? getRepresentativePrice(car)) + ((cy.insuranceMin + cy.insuranceMax) / 2 / 12) + cy.roadTax
    ...
    <section className="car-section">
      ...
      <div className="car-cost">
        <div className="car-cost-col">
          ...
          {onRoadRows.map(function(r) { return <div key={r.k} className="car-cost-row"><span className="k">{r.k}</span><span className="v">{r.v}</span></div> })}
          <div className="car-cost-total"><span className="lab">Month 1 outlay</span><span className="v">{fmtGBP(onRoadTotal)}</span></div>
        </div>
        <div className="car-cost-col">
          <h4><span>Ownership cost</span><span className="meta-pill">48 months</span></h4>
          {ownershipRows.map(function(r) { return <div key={r.k} className="car-cost-row"><span className="k">{r.k}</span><span className="v">{r.v}</span></div> })}
          <div className="car-cost-row"><span className="k">Depreciation (4 years)</span><span className="v">−{fmtGBP(ownershipDepreciation)}</span></div>
          <div className="car-cost-row positive"><span className="k">↑ Est. resale value (48 months)</span><span className="v">{fmtGBP(ownershipResale)}</span></div>
          <div className="car-cost-total"><span className="lab">True 48-month cost</span><span className="v">{fmtGBP(ownershipTotal)}</span></div>
        </div>
      </div>
    </section>
  )
...
function RetailerModule(props) {
  const car = props.car
  const answers = props.answers
  const cards = useMemo(function() { return getRetailerCards(car, answers) }, [car, answers])
  ...
  return (
    <section className="car-section">
      ...
      <p className="car-section-lede">Live listings near you. Estimates updated weekly — click through for the real count.</p>
      <div className="car-retailers">
        {cards.map(function(c) {
          return (
            <a key={c.id} href={c.url} target="_blank" rel="noopener noreferrer" className="car-retailer-card" style={{ '--accent': c.accent }}>
              ...
              {c.avgPrice && <div className="car-retailer-avg">Avg. price {fmtGBP(c.avgPrice)}</div>}
              <div className="car-retailer-cta">Search {c.name}<span className="arrow" aria-hidden="true"></span></div>
            </a>
          )
        })}
      </div>
      <div className="car-section-foot">Searching in {postcode} {radLabel}.</div>
    </section>
  )
}
function InsuranceModule(props) {
  const car = props.car
  const answers = props.answers
    ...
    <section className="car-section">
      ...
      <div className="car-insurance">
        <div className="car-insurance-cost">
          ...
          <div className="disclaimer">Estimates based on this car's risk band. Your real quote depends on driver profile.</div>
        </div>
        <div className="car-insurance-cta">
          ...
          <div className="car-insurance-direct">
            ...
            {direct.map(function(d) {
              return (
                <a key={d.id} href={d.url} target="_blank" rel="noopener noreferrer" className="car-insurance-direct-btn">
                  {d.name}
    ...
    <section className="car-section">
      ...
      <div className="car-finance">
        <div className="car-finance-cost">
          ...
          <div className="disclaimer">Real rates depend on your credit profile. Quote engines below run a soft search.</div>
        </div>
        <div className="car-finance-cta">
          ...
          <div className="car-finance-direct">
            ...
            {direct.map(function(d) {
              return (
                <a key={d.id} href={d.url} target="_blank" rel="noopener noreferrer" className="car-finance-direct-btn">{d.name}</a>
              )
            })}
          </div>
        </div>
...
function KnownIssuesModule(props) {
  const car = props.car
  ...
  if (notes.length === 0) {
    return (
      <section className="car-section">
        ...
        <p className="car-section-lede">Nothing significant flagged on this generation. Standard pre-purchase inspection still recommended.</p>
      </section>
    )
  }
  return (
    <section className="car-section">
      ...
      <p className="car-section-lede">Things to check before buying — sourced from owner forums and MOT data.</p>
...
function PriceRail(props) {
  const car = props.car
  const repPrice = props.repPrice
  const cy = props.cy
  const trueCost = props.trueCost
  const resale = props.resale
  const matchScore = props.matchScore
  const isFinance = props.isFinance
  const onSearchStock = props.onSearchStock
  const onGetFinance = props.onGetFinance
  const onGetInsurance = props.onGetInsurance

  const monthly = isFinance && cy?.financeMonthly ? cy.financeMonthly : null
  const month1 = isFinance ? (cy.deposit + cy.financeMonthly + cy.insuranceMonthly + cy.roadTax) : null

  return (
    <div className="car-rail-block car-rail-price">
      <div className="car-rail-price-v">{fmtGBP(repPrice)}</div>
      <div className="car-rail-price-meta">Representative price · {car.generationYears?.split(/[-–—]/)[1]?.trim() || ''} · ~48,000 miles</div>
      <div className="car-rail-stats">
        <div className="row"><span className="k">Match score</span><span className="v"><strong>{matchScore}</strong> / 100</span></div>
        {monthly && <div className="row"><span className="k">HP monthly</span><span className="v"><strong>{fmtGBP(monthly)}</strong> / mo</span></div>}
        {month1 && <div className="row"><span className="k">Month 1 outlay</span><span className="v"><strong>{fmtGBP(month1)}</strong></span></div>}
        <div className="row"><span className="k">True 48-month cost</span><span className="v"><strong>{fmtGBP(trueCost)}</strong></span></div>
        <div className="row"><span className="k">Est. resale value</span><span className="v positive"><strong>−{fmtGBP(resale)}</strong></span></div>
      </div>
      <button className="btn lg" onClick={onSearchStock}>Search available stock<span className="arrow" aria-hidden="true"></span></button>
      {onGetFinance && <button className="btn ghost" onClick={onGetFinance}>Get finance quote<span className="arrow" aria-hidden="true"></span></button>}
      <button className="btn ghost" onClick={onGetInsurance}>Get insurance quote<span className="arrow" aria-hidden="true"></span></button>
      <button className="btn ghost disabled" disabled title="Coming soon — sign in to save cars">Save this car</button>
    </div>
  )
}

function VerdictRail(props) {
  const car = props.car
  const matchScore = props.matchScore
  ...
  const depreciationLine = dep === 'Low' ? 'predictable running costs and strong residuals' : dep === 'Medium' ? 'middle-of-pack residuals' : 'higher-than-average value loss'

  return (
    <div className="car-rail-block car-rail-verdict">
      <div className="kicker">◆ Motifi verdict</div>
      <div className="score">{matchScore}</div>
      <p>The <strong>{car.make} {car.model}</strong> is a {reliabilityStrength} all-round performer in your search. {reliabilityStrength === 'excellent' ? 'Excellent' : reliabilityStrength === 'strong' ? 'Strong' : 'Workable'} reliability, and {depreciationLine}.</p>
      {parseNotes(car.notes).length > 0 && <p className="muted">Check the known-issues panel below before pulling the trigger.</p>}
    </div>
  )
}

function OtherMatchesRail(props) {
  ...
  return (
    <div className="car-rail-block car-rail-others">
      ...
      {cars.map(function(c, i) {
        ...
        return (
          <button key={`${c.make}-${c.model}-${i}`} className="car-rail-other" onClick={() => onSelectCar?.(c)}>
